...
lmbd = np.zeros(31)
for B in range(1,31):
    lmbd[B] = (model.f_k[B*3]*model.c_k[B*3]+model.f_k[B*3-1]*model.c_k[B*3-1]+model.f_k[B*3-2]*model.c_k[B*3-2]) / (model.c_k[B*3]+model.c_k[B*3-1]+model.c_k[B*3-2])

shelf_sorted = np.flip(np.argsort(lmbd))[:-1]

def algo_1(shelf_sorted):
    i = 1
    S = []
    
    i_star = shelf_sorted[i]
